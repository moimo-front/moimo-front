...
const ProfileModal = ({ isOpen, onClose, userInfo }: ProfileModalProps) => {
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      const img =
        userInfo.profile_image || (userInfo as any).profile_image || null;
      setPreviewImage(img);
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              {!isReadOnly && (
                <p className="text-[10px] text-gray-400 text-center block">
                  최소 3개이상 선택해주세요!
                </p>
              )}
            </div>
            <div className="grid grid-cols-4 gap-2">
              {allInterests?.map((interest) => (
                <button
                  ...
                  disabled={
                    isReadOnly && !selectedInterests.includes(interest.id)
                  }
                  className={cn(
